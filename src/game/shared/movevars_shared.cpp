...
ConVar	sv_maxspeed		( "sv_maxspeed", "640", FCVAR_NOTIFY | FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY);

#ifdef _XBOX
	ConVar	sv_accelerate	( "sv_accelerate", "7", FCVAR_NOTIFY | FCVAR_REPLICATED);
#else
#ifdef CRUN_DLL
	ConVar	sv_accelerate	( "crun_accelerate", "4.5", FCVAR_NOTIFY | FCVAR_REPLICATED );
#else
...
#ifdef CRUN_DLL
ConVar	sv_airaccelerate(  "sv_airaccelerate", "200", FCVAR_NOTIFY | FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY );    
#else
...
#if defined(INFESTED_DLL) || defined(CRUN_DLL)
...
ConVar	sv_backspeed	( "sv_backspeed", "0.78", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY, "How much to slow down backwards motion" );
